...
import '../requests/GetResponseRequest.dart';
...
class PdfScreen extends StatefulWidget {
  ...
  const PdfScreen({super.key, required this.book, required this.pages, required this.extractor, required this.document});
}
...
class _PdfScreenState extends State<PdfScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (topHeight == 0){
      topHeight = 50+MediaQuery.of(context).padding.top;
    }

    Future<String> getDifinition(String concept) async {
      String text;
      final message = "What does this mean: ${concept}\nPlease elaborate as much as possible";
      ...
      GetLastProcessedPageRequest request3 = GetLastProcessedPageRequest(
          id: widget.book.postgId);
      try {
      ...
      on Exception {
        ...
        ErrorAnouncer.showErrorDialog(text, context);
        return "";
      }
      ...
      try {
        ...
        if (pages.isNotEmpty) {
          CreatePagesRequest request1 = CreatePagesRequest(
              documentId: widget.book.postgId, pages: pages);
          await http.post(
            Uri.parse(url1),
            headers: {'Content-Type': 'application/json'},
      ...
      try {
        GetResponseRequest request2 = GetResponseRequest(
            documentId: widget.book.postgId, question: message);
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            actions: [
              Expanded(
                  ...
                  child: InkWell(
                      onTap: () async {
                        String concept = getText();
                        String definition = await getDifinition(concept);
                        Concept concept1 = Concept(concept: concept, definition: definition);
                        await Concepts.addConcept(widget.book.postgId, concept1);
                      },
                      child: Container(
                          ...
                          child: isExtraction ? Image.asset(
                            PicAbbrvs.vocab,
              ...
              Expanded(
                flex: 2,
                child: Center(
                  child: Text("${page+1}/${widget.pages}"),
                )
              ),
              Expanded(
                flex: 2,
        ...
        body: GestureDetector(
          onLongPressStart: (details){
            ...
            double y = details.globalPosition.dy-topHeight;
          ...
          child: Stack(
            children: [PerfectPdfRenderer(
              ...
              defaultPage: widget.book.currentPage,
              pages: widget.pages,
              onMaxWidthSet: (int maxWidthInt){
                initZoom = screenWidth/widget.document.pages[maxWidthInt].size.width;
              },
              extractionEnd: (){
                isExtraction = false;
              },
              setOffsets: (List<Object?> yOffsets){
                offsets = List.filled(yOffsets.length, List.empty());
              ...
              onPageChanged: (int? page) async{
                this.page = page!;
                widget.book.currentPage = this.page;
                setState((){});
              },
  ...
  bool setInitHandlePosition(double x, double y){
    ...
    for (int i = 0; i < offsets.length-2; i ++){
      if (i == offsets.length - 3 || y > offsets[i][0] && y <= offsets[i+1][0]){
        ...
        pageInd = i;
        cache[page] = widget.extractor.extractTextLines(startPageIndex: page, endPageIndex: page);
        break;
      }
    }
  ...
  @override
  void dispose() {
    Books.updatePage(widget.book.path, page);
    super.dispose();
  }
}